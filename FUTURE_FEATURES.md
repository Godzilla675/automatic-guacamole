...
- [ ] **Bug: Projectile takeDamage missing**: Projectiles lack the takeDamage method because they are created as simple objects without prototype methods. (Status: Bug, agents must fix)
- [ ] **Bug: Wood Door blocks missing from BLOCKS**: The DOOR_WOOD_BOTTOM definition is missing from the global BLOCKS array. (Status: Bug, agents must fix)
- [ ] **Bug: Auto-Save System property missing**: The autoSaveTimer property cannot be verified on the game object, indicating Auto-Save might not be correctly integrated or tracked as required. (Status: Bug, agents must fix)

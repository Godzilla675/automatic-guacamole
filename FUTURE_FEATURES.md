...
- [ ] **Bug: Canvas Pattern Creation in Headless Contexts**: `createPattern` throwing errors in headless test environments lacking full HTMLCanvasElement rendering backends. (Status: Known headless canvas limit, agents must fix)
- [ ] **Bug: Flaky Chunk Loading Test under high concurrency**: Chunk loading boundary test under concurrent test runner load occasionally failing due to chunk generation timing. (Status: Known test runner timing issue, agents must fix)
- [ ] **Bug: JSDOM Mocha test global.Game reference loss**: `global.Game` becoming undefined during JSDOM Mocha `beforeEach` hooks in headless test suites requiring fallback assignment (`if (!global.Game) global.Game = dom.window.Game || window.Game`). (Status: Known test environment limit, agents must handle in test hooks)

...
- [ ] **Bug: Player Speed Soul Sand**: In js/player.js, player movement speed (this.speed = 4.3) is currently not affected by BLOCK.SOUL_SAND's slow: true property because the slowdown logic is missing from the physics integration loop. (Status: Missing implementation, agents must fix)
...
- [ ] **Bug: Slab and door collision tests**: Slab and door collision tests randomly fail during automated testing due to Perlin noise terrain generation overlapping the test coordinates. (Status: Requires clearing test area, agents must fix)
...
- [ ] **Bug: Jungle Biome Generation**: In verification/verify_world_gen.js, testing Jungle biome generation requires mocking Perlin noise to return > 0.6 (e.g., 0.7) to correctly satisfy the humidity > 0.6 condition. (Status: Missing implementation, agents must fix)
...
- [ ] **Bug: Rideable Pigs Missing**: The Rideable Pigs feature is missing from the codebase. (Status: Missing implementation, agents must fix)
- [ ] **Bug: Auto-Save Missing**: The Auto-Save feature is missing from the codebase. (Status: Missing implementation, agents must fix)
- [ ] **Bug: Chat History Toggle Missing**: The Chat History Log toggle is missing from the codebase. (Status: Missing implementation, agents must fix)

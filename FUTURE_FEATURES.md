...
### 2. Block System
- [ ] **Bug: Top Slabs**: Placement in upper half of block missing. (Status: Broken, setBlockMeta is not a function in world.js, agents must fix)
...
- [ ] **Bug: Glowstone Crafting**: Light emitting block found in the Nether. (Status: Broken, incorrect glowstone recipe or missing result, agents must fix)
...
- [ ] **Bug: Shields Logic**: Add craftable shields to block incoming entity attacks and projectiles. (Status: Broken, blocking logic and durability not properly integrated in player.js and game.js, agents must fix)
...
- [ ] **Bug: Missing Hitboxes for Fences, Stairs, and Slabs**: Incomplete block hitboxes allowing players to phase through them. (Status: Broken, window.Physics.getBlockBoundingBox is not a function, agents must fix)
...
- [ ] **Bug: Potion effects logic missing**: Potion items exist but effect consumption is not fully implemented. (Status: Broken, player.useItem or consumption logic is missing, agents must fix)
...
- [ ] **Bug: Rideable Pigs missing**: Allow players to ride pigs using saddles and carrot on a stick. (Status: Broken, player is not riding after interaction, agents must fix)
...
- [ ] **Bug: Shield missing player logic**: Shield exists as a block, but its blocking logic and integration within js/player.js are missing. (Status: Broken, isBlocking function missing on player, agents must fix)
...
- [ ] **Bug: Potion Consumption Effect Logic**: Potion items exist but player useItem and status effect consumption logic is incomplete. (Status: Broken, consumption logic missing in player.js, agents must fix)
- [ ] **Bug: Rideable Pigs Dismount and Control**: Rideable pig entity state interaction does not maintain riding state on player. (Status: Broken, player state reset after interaction, agents must fix)
